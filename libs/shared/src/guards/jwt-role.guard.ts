import { CanActivate, ExecutionContext, ForbiddenException, UnauthorizedException, mixin, Type, BadRequestException, InternalServerErrorException } from '@nestjs/common'
import * as jose from 'node-jose'
import fetch from 'node-fetch'

export const JwtRoleGuard = (allowedRoles?: string[]): Type<CanActivate> => {
    class JwtRoleGuardMixin implements CanActivate {
        public url = process.env.AUTH_SERVICE_JWKS_URL || 'http://auth-service:3000/jwks.json'
        ...
        async canActivate(context: ExecutionContext): Promise<boolean> {
            ...
            if (decodedHeader?.alg !== 'RS256') {
                throw new BadRequestException('Invalid JWT algorithm, expected RS256');
            }
        ...
        private async getKey(kid?: string): Promise<jose.JWK.Key> {
            ...
            if (!key) throw new InternalServerErrorException('JWT key not found in JWKS')
            return key
        }
    }
