...
export class MealRepeatCalculator {
  ...
  private static checkWeeklyRepeat(
    ...
    if (!config.daysOfWeek || config.daysOfWeek.length === 0) {
      return relativeDate.getDay() === startDate.getDay();
    }

    return config.daysOfWeek.includes(relativeDate.getDay() as DayOfWeek);
  }
  ...
  private static checkMonthlyRepeat(
    ...
    const monthsDiff = (relativeDate.getFullYear() - startDate.getFullYear()) * 12 + 
                      (relativeDate.getMonth() - startDate.getMonth());
    ...
    return relativeDate.getDate() === startDate.getDate();
  }
