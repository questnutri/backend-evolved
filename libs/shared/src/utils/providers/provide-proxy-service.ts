import { generateProxyServiceProvider } from "../proxy";
...
export const provideProxyService = (serviceName: string) => {
    switch (serviceName) {
        ...
        case PROXY_NAMES.GAME_SERVICE_PROXY_NAME:
            return generateProxyServiceProvider(
                PROXY_NAMES.GAME_SERVICE_PROXY_NAME,
                process.env.DEV_GAME_QUEUE_NAME || 'game_queue'
            );
        case PROXY_NAMES.NOTIFICATION_SERVICE_PROXY_NAME:
            return generateProxyServiceProvider(
                PROXY_NAMES.NOTIFICATION_SERVICE_PROXY_NAME,
                process.env.DEV_NOTIFICATION_QUEUE_NAME || 'notification_queue'
            );
        default:
